...
NOTE: The `runner()` bean is a `CommandLineRunner` but it is only instantiated if the application is *not* running in the "test" profile. For more information on profiles see link:/understanding/profiles[Understanding Profiles]
